...
router.post("/delete-all-notifications", authMiddleware, async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message: "All notifications clear",
      data: updatedUser,
    });
  } catch (error) {
    console.log(error);
...
router.get("/get-all-approved-doctors", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error getting all doctors", success: false, error });
  }
});
...
router.get("/check-booking-availability", authMiddleware, async (req, res) => {
  try {
